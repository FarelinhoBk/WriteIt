    ...
    <form method="post" action=<%=inclusao?"Incluir":"Alterar"%> >
      ...
      <input type="hidden" value=<%=t==null?"":t.getId()%> name="id" />
        <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
          <input class="mdl-textfield__input" type="text" name="nome" value="<%=t==null?"":t.getNome()%>" required>
          <label class="mdl-textfield__label" for="nome">Nome</label>
        </div>
				<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
					<input class="mdl-textfield__input" type="text" name="descricao" value="<%=t==null?"":t.getDescricao()%>" required>
					<label class="mdl-textfield__label" for="descricao">Descricao</label>
				</div>
        <div class="mdl-textfield mdl-js-textfield">
          <input class="mdl-textfield__input" type="date" name="dataLimite" value="<%=t==null?"":t.getDataLimite()%>" required>
        </div>
        <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
          <input class="mdl-textfield__input" type="number" step="0.01" name="valor" value="<%=t==null?"0":t.getValor()%>" required>
          <label class="mdl-textfield__label" for="valor">Valor</label>
        </div>
    <%
    // Se nao esta em inclusao
    if(!inclusao){ %>
      <div>Situacao:<select name="situacao" value="<%=t==null?"1":t.getSituacao()%>">

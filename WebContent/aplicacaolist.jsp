...
<body>
	...
	<div>
		...
		%>
		<a id="task-create"
			class="mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored mdl-color-text--white"
			href="Manutencao?entidade=aplicacao&idTarefa=<%=idTarefa%>"> <i
			...
			<tbody>
				<%
					...
					for (Aplicao a : aplicaos) {
						...
						if (usu.isFreelancer() & usu.getId() != criador.getId())
							continue;
				%>
				<tr>
					...
					%>
					<td><a id="task-???"
						href="Manutencao?entidade=aplicacao&id=<%=a.getId()%>">Alterar</a></td>
					<td><a id="task-delete"
						href="Deletar?entidade=aplicacao&id=<%=a.getId()%>">Deletar</a></td>
					<%
						}
					%>
				</tr>
